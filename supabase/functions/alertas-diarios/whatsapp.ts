// Adapter de envio de WhatsApp para uso dentro da Edge Function (Deno).
// Espelha a interface de /lib/whatsapp.ts do app Next.js — trocar de provedor
// (Twilio, Z-API, Meta Cloud API) exige mudar somente este arquivo.
...
async function sendViaTwilio(config: WhatsappConfig, telefone: string, mensagem: string): Promise<WhatsappSendResult> {
  ...
  if (!res.ok) return { ok: false, error: `Twilio respondeu ${res.status}` };
  return { ok: true };
}

async function sendViaZApi(config: WhatsappConfig, telefone: string, mensagem: string): Promise<WhatsappSendResult> {
...
async function sendViaMetaCloud(config: WhatsappConfig, telefone: string, mensagem: string): Promise<WhatsappSendResult> {
  if (!config.apiUrl || !config.apiToken) {
  ...
  if (!res.ok) return { ok: false, error: `Meta Cloud API respondeu ${res.status}` };
  return { ok: true };
}

export function whatsappConfigFromEnv(env: (key: string) => string | undefined): WhatsappConfig {
  return {
